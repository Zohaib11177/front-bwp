...
import ProfileUpdateAction from "Redux/V1/Profiles/Put/ProfileUpdateAction";
...
function* profileUpdate(data) {
    try {
        ToastHelper.info();
        const response = yield ProfileUpdateService.profileUpdateBody(
            data.request.form
        );
        if (response.success) {
...
export function* ProfileUpdateSaga() {
    yield takeEvery(PROFILE.PROFILE_PUT, profileUpdate);
}
